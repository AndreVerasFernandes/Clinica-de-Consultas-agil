...
def add_appointment(appointments, patients, patient_index, date, time, specialty):
    ...
    if appointment_date < datetime.today():
        print("Você não pode agendar consultas no passado.")
        return appointments

    for appointment in appointments:
        if appointment['date'] == appointment_date.strftime('%d/%m/%Y') and appointment['time'] == appointment_time.strftime('%H:%M'):
...
def main():
    ...
    while True:
        ...
        if option == 1:
            ...
            appointments = add_appointment(appointments, patients, patient_index, date, time, specialty)
